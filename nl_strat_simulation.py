...
steps = 800000 #At 800000 steps, takes many hours to run
save_freq = 15
save_max = 15
...
wall['g'] = sigmoid(-(x - 0.02*L), a=2*epsilon) + sigmoid(x - 0.98*L, a=2*epsilon)
...
rho.meta['z']['parity'] = +1
rho['g'] = sw.dens0(C_w * C['g'], T_w * T['g'])
...
params = [Nx, Nz, delta, epsilon, mu, eta, h, U, L, H, B, T, par, S, nu, rho0, wall, scale, z0, Delta, b, strat, rho]
param_names = ['Nx', 'Nz', 'delta', 'epsilon', 'mu', 'eta', 'h', 'U', 'L', 'H', 'B', 'T', 'par', 'S', 'nu', 'rho0', 'wall', 'scale', 'z0', 'Delta', 'b', 'strat', 'rho']
